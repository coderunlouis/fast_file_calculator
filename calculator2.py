...
def process_single_file(
    ...
    filter_or_conditions: List[List[Dict[str, Any]]] = None
):
    # 使用惰性读取
    ...
    if filter_or_conditions:
        for or_group in filter_or_conditions:
            # 构建OR条件
            or_filter = pl.lit(False)  # 初始化为 False
            for condition in or_group:
                ...
                if op == '==':
                    ...
                    or_filter |= pl.col(column).is_in(value)
            
            # 应用OR条件
            df = df.filter(or_filter)
...
def process_all_at_once(
    ...
    if filter_or_conditions:
        for or_group in filter_or_conditions:
            ...
            # 应用OR条件
            full_df = full_df.filter(or_filter)
...
class MyDataFrame:
    def __init__(self, directory):
        ...
        self.sum_columns = []
        self.mean_columns = []
        self.filter_conditions = []  # 存储所有过滤条件
        self.filter_or_conditions = []  # 新增OR条件列表
    ...
    def filter_or(self, conditions: List[Dict[str, Any]]):
        """
        添加OR过滤条件
        
        Args:
            conditions (List[Dict]): 多个过滤条件的列表
                每个条件是一个字典，包含 'column', 'op', 'value'
        
        Returns:
            self: 返回当前对象，支持链式调用
        """
        self.filter_or_conditions.append(conditions)
        return self
    
    def compute(self, method='separate'):
        """
        触发计算并返回结果，支持AND和OR过滤条件
        ...
        if method == 'separate':
            result = process_files_separately(
                ...
                self.sum_columns, 
                self.mean_columns,
                self.filter_conditions,  # 传递AND过滤条件
                self.filter_or_conditions  # 传递OR过滤条件
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 链式调用：分组、复杂OR过滤、求和、求平均
        result_separate = (df.groupBy(["省份", "班级"])
                           .filter_in("省份", ["四川", "山东"]) 
                    .filter_or([
                        {"column": "省份", "op": "==", "value": "四川"},
                        {"column": "语文", "op": ">=", "value": 200}
                    ])  # 省份是四川 OR 语文大于等于60
                    .sum(["语文", "数学"])
                    .mean(["化学", "生物", "地理"])
        ...
        # 另一个复杂OR过滤的例子
        result_or_math = (df.groupBy(["省份", "班级"])
                          .filter_in("省份", ["四川", "山东"]) 
                    .filter_or([
                        {"column": "数学", "op": ">", "value": 90},
                        {"column": "数学", "op": "<", "value": 60}
                    ])  # 数学大于90 OR 数学小于60
                    .sum(["语文", "数学"])
                    .mean(["化学", "生物", "地理"])
                    .compute(method='separate'))
        ...
        temp_file1 = os.path.join(res_dir, "result_separate_or.csv")
        temp_file2 = os.path.join(res_dir, "result_all_at_once_or.csv")
        temp_file3 = os.path.join(res_dir, "result_or_math.csv")

        result_separate.write_csv(temp_file1)
        result_all_at_once.write_csv(temp_file2)
        result_or_math.write_csv(temp_file3)

        print(f"分步计算结果保存至: {temp_file1}")
        print(f"一次性计算结果保存至: {temp_file2}")
        print(f"另一个复杂OR过滤的例子结果保存至: {temp_file3}")
